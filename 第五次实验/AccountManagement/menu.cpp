...
{
	cout << "----------上机----------" << endl;
	char lName[20], lPwd[10];
	cout << "请输入上机卡号（长度为1~18）：";
	cin >> lName;
	cout << "请输入上机密码（长度为1~8）：";
	cin >> lPwd;
	Card *pCard = delogon(lName, lPwd);
	if (pCard == NULL)
	{
		cout << "上机失败！" << endl;
		return;
	}
	if (pCard->nStatus == 0 && pCard->fBalance > 0 &&pCard->nDel == 0)
	{
		...
		pCard->nUseCount++;
		char time[20];
		timeToString(pCard->tLast, time);
		
		cout << "----------上机信息如下----------" << endl;
		cout << "卡号\t余额\t上机时间" << endl;
		cout << pCard->aName << '\t' << pCard->fBalance << '\t' << time << endl;
		saveLogonCard(CARDPATH);
		addBilling(BILLINGPATH, pCard);
	}
	else if (pCard->nStatus != 0 || pCard->nDel == 0)
	{
